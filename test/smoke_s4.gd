...
# (3) 기본 명령(주변) 상태의 동료가 무녀에게서 먼 적에게 먼저 교전 개시하는지,
# (4) 모여라(키 4) 명령에 동료들이 무녀 곁으로 집결하는지 헤드리스로 검증한다.
...
class Monitor extends Node:
	...
	func _process(_delta: float) -> void:
		...
		if _frames > MAX_FRAMES:  # pause 분기보다 먼저 — 결과 화면(S7) 정지에서 행 방지
			printerr("SMOKE FAIL — 제한 프레임 초과 (phase %s, sweep %s, melee %s, charge %s)" % [
				_phase, str(_sweep_seen), str(_melee_seen), str(_charge_seen),
			])
			get_tree().quit(1)
			return
	...
	# 기본 명령(주변) 돌격 — 무녀에게서 먼 적과 교전 접촉(공격 사거리 진입)을 관찰.
	# 종전 leash(260)로는 불가능한 거리라, 관찰되면 기본 성향 적극화가 입증된다.
	func _observe_far_charge(munyeo: Node2D, companions: Array) -> void:
		if _charge_seen or munyeo.command != Command.NEARBY:
			return
		for companion in companions:
			if companion.downed:
				continue
			for enemy in get_tree().get_nodes_in_group("enemy"):
				if munyeo.global_position.distance_to(enemy.global_position) \
						<= FAR_CHARGE_DISTANCE:
					continue
				if companion.global_position.distance_to(enemy.global_position) \
						<= companion.attack_range:
					_charge_seen = true
					print("기본 돌격 확인 — %s가 무녀에게서 %.0fpx 떨어진 적과 교전 (frame %d)" % [
						companion.display_name,
						munyeo.global_position.distance_to(enemy.global_position), _frames,
					])
					return

	# 혼불을 주워(보유) 동료에게 다가가 분배 — 아무 동료나 Lv 2 도달까지.
	# 성향(광역 베기·근접 교전·기본 돌격)까지 관찰돼야 모여라 단계로 넘어간다(모여라 후엔 교전 안 함).
	func _level_companion(munyeo: Node2D, companions: Array) -> void:
		for companion in companions:
			if companion.level >= 2 and _sweep_seen and _melee_seen and _charge_seen:
	...
	func _check_rally(munyeo: Node2D, companions: Array) -> void:
		...
		if _rally_frames < 10:  # 우연한 통과 방지 — 10프레임 유지 확인
			return
		print("SMOKE OK — 분배 레벨업 + 모여라 집결 + 광역 베기/근접/기본 돌격 성향 (frame %d)" % _frames)
		get_tree().quit(0)
